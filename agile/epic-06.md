...
## Goal
Fine-tune **all** PhasorAtom parameters jointly by minimizing KL-divergence between teacher
and student logits on a large corpus. Stage 1 aligns each layer independently; Stage 2
eliminates the error accumulation that happens when slightly-off layers compound across
18 transformer blocks. This is the step that makes the student model actually *behave*
like the teacher.
...
2. As an engineer, I want a student model assembled from `.atom` files that can run a full forward pass in PyTorch so I can backpropagate through the phasor synthesis.
3. As a researcher, I want perplexity and KL-divergence metrics tracked during training so I know when to stop.

## Acceptance Criteria
- [ ] `PhasorStudentModel` assembles a full Gemma 2B forward pass using `PhasorAtomLayer` for every target weight, loaded from Stage 1 `.atom` files.
...
## Quality Gate
This is the **distillation go/no-go decision**. If KL-divergence ≤ 0.05 and PPL delta
≤ +0.35, the distilled phasor weights are behaviorally equivalent to the teacher. Proceed
to Epic 07 (full-model encoder) using distilled `.atom` files as the encoding source.
If the gate fails, options: (a) increase Stage 2 steps/lr, (b) revisit Stage 1 K profile,
(c) hybrid approach — use direct codec for layers where distillation struggles.
Document the verdict in `agile/stage2_verdict.md`.
